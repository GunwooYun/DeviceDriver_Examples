...
{
	int dev;
	char key_no, old_key_no = 0;
	char led_no;
	char timer_val;
	int ret;
	int cnt = 0;
	int loopFlag = 1;
	struct pollfd Events[2]; // poll
	char inputString[80];
	keyled_data info; // define in header file
	...
	printf("Author:윤건우\n");
    timer_val = atoi(argv[2]); // character to integer
	info.timer_val = timer_val;

//	dev = open(DEVICE_FILENAME, O_RDWR | O_NONBLOCK);
	dev = open(DEVICE_FILENAME, O_RDWR ); // bloking mode
	if(dev < 0)
	{
		perror("open");
		return 2;
...
//  		printf("poll time out : %d\n",cnt++);
		...
		if(Events[0].revents & POLLIN)  //dev : keyled
		{
			...
			printf("key_no : %d\n",key_no);
			if(key_no != old_key_no){ // No press same key
				switch(key_no) 
				{
					case 1:
						printf("TIMER STOP! \n");
